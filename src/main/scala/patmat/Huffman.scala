...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def incr(sub: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = sub match {
      case Nil	   => acc
      case x :: xs => incr(xs.filter(_ != x), acc :+ (x, 1 + xs.count(_ == x)))
    }
    incr(chars, List())
  }
  ...
  //def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = a ::: b //<-- Так красивее смотрится и тоже работает
